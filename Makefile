...
source: clean
	...
	git archive --prefix="avocado-cloudtest-$(COMMIT)/" -o "SOURCES/avocado-cloudtest-$(VERSION)-$(SHORT_COMMIT).tar.gz" HEAD

source-release: clean
	if test ! -d SOURCES; then mkdir SOURCES; fi
	git archive --prefix="avocado-cloudtest-$(RELEASE_COMMIT)/" -o "SOURCES/avocado-cloudtest-$(VERSION)-$(RELEASE_SHORT_COMMIT).tar.gz" $(VERSION)
...
source-pypi: clean
	...
	git archive --prefix="avocado-cloudtest/" -o "PYPI_UPLOAD/avocado-cloudtest-$(VERSION).tar.gz" $(VERSION)

pypi: source-pypi develop
	cp avocado_cloudtest.egg-info/PKG-INFO PYPI_UPLOAD/PKG-INFO
	@echo
...
srpm: source
	...
	mock -r $(MOCK_CONFIG) --resultdir BUILD/SRPM -D "commit $(COMMIT)" --buildsrpm --spec avocado-cloudtest.spec --sources SOURCES

rpm: srpm
	if test ! -d BUILD/RPM; then mkdir -p BUILD/RPM; fi
	mock -r $(MOCK_CONFIG) --resultdir BUILD/RPM -D "commit $(COMMIT)" --rebuild BUILD/SRPM/avocado-cloudtest-$(VERSION)-*.src.rpm

srpm-release: source-release
	if test ! -d BUILD/SRPM; then mkdir -p BUILD/SRPM; fi
	mock -r $(MOCK_CONFIG) --resultdir BUILD/SRPM -D "commit $(RELEASE_COMMIT)" --buildsrpm --spec avocado-cloudtest.spec --sources SOURCES

rpm-release: srpm-release
	if test ! -d BUILD/RPM; then mkdir -p BUILD/RPM; fi
	mock -r $(MOCK_CONFIG) --resultdir BUILD/RPM -D "commit $(RELEASE_COMMIT)" --rebuild BUILD/SRPM/avocado-cloudtest-$(VERSION)-*.src.rpm
...
clean:
	...
	rm -rf avocado_cloudtest.egg-info
...
smokecheck: clean develop
	./scripts/avocado list | head -n10

check: clean develop check_cyclical
	selftests/checkall
...
spell:
	pylint --errors-only --disable=all --enable=spelling --spelling-private-dict-file=spell.ignore * && echo OK
