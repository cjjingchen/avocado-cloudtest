...
class TestStatus(object):
    ...
    def finish(self, proc, started, timeout, step):
        ...
        test_state['time_end'] = time.time()
        test_state['time_elapsed'] = test_state['time_end'] - started
...
class TestRunner(object):
    ...
    def run_test(self, test_factory, queue, summary, job_deadline=0):
        ...
        if test_factory[1]['ct_params'].get('abort_on_error', 'no') in 'yes' \
           and test_state.get('status') in ('ERROR', 'FAIL'):
            ctrl_c_count += 1
