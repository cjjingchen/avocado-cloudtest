    ...
    * end_job
    ...
    def __init__(self, basedir=None, log_packages=None, profiler=None):
        ...
        if os.path.isfile(commands_file):
            log.debug('Commands configured by file: %s', commands_file)
        ...
        if os.path.isfile(files_file):
            log.debug('Files configured by file: %s', files_file)
        ...
        if os.path.isfile(profiler_file):
            ...
            log.debug('Profilers configured by file: %s', profiler_file)
            if not self.profilers:
                self.profiler = False

            if self.profiler is False:
                if not self.profilers:
                    log.debug('Profiler disabled: no profiler commands configured')
                else:
...
def collect_sysinfo(args):
    ...
    log.debug("Logged system information to %s", basedir)
