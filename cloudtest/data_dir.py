...
_SYSTEM_WIDE_ROOT_PATH = '/usr/share/avocado-cloudtest'
if os.path.isdir(_SYSTEM_WIDE_ROOT_PATH):
    _INSTALLED_SYSTEM_WIDE = len(os.listdir(_SYSTEM_WIDE_ROOT_PATH)) > 0
else:
    _INSTALLED_SYSTEM_WIDE = False

if _INSTALLED_SYSTEM_WIDE:
    # avocado-cloudtest is installed
    _ROOT_PATH = _SYSTEM_WIDE_ROOT_PATH
else:
    # we're running from source code directories
    cloudtest_init = sys.modules['cloudtest'].__file__
    cloudtest_dir = os.path.realpath(os.path.dirname(cloudtest_init))
    _ROOT_PATH = os.path.dirname(cloudtest_dir)

cloudtest_init = sys.modules['cloudtest'].__file__
CLOUDTEST_MODULE_DIR = os.path.realpath(os.path.dirname(cloudtest_init))
ROOT_DIR = os.path.abspath(_ROOT_PATH)

CLOUDTEST_CONFIG_DIR = os.path.join(ROOT_DIR, 'config')
CLOUDTEST_TEST_DIR = os.path.join(ROOT_DIR, 'tests')
...
PERFORMANCE_CASE_DIR = os.path.join(ROOT_DIR, 'tests', 'performance', 'rally-jobs')
CLOUDTEST_MODULE_TEST_DIR = os.path.join(CLOUDTEST_MODULE_DIR, 'tests')
CLOUDTEST_INTEGRATE_TEST_DIR = os.path.join(CLOUDTEST_MODULE_TEST_DIR, 'integrate')
