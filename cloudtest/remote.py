...
def wait_for_login(client, host, port, username, password, prompt,
    ...
    return remote_login(client, host, port, username, password, prompt,
                        linesep, log_filename, internal_timeout, interface, use_key=use_key)
...
class RemoteRunner(object):
    ...
    def copy_file_to(self, local_path, remote_path, username='root', port='22'):
        """
        Method to copy local file/folder to remote host.

        :param local_path: the path of local file/folder
        :param remtoe_path: the path on remote host
        :param username: username to use for authentication
        :param port: scp port number
        """
        command = ("scp -v -o UserKnownHostsFile=/dev/null "
                   "-o StrictHostKeyChecking=no "
                   "-r -P %s %s %s@\[%s\]:%s" %
                   (port, local_path, username, self.host, remote_path))
        password_list = []
        password_list.append(self.password)
        try:
            session = aexpect.Expect(command)
            _remote_scp(session, password_list)
        finally:
            session.close()

class RemoteUtils(object):
    """
    Class to provide a utils
    """

    def __init__(self, session=None, host=None, username=None,
                 password=None, use_key=False):
        if session is None:
            session = RemoteRunner(host=host, username=username,
                                   password=password, use_key=use_key)
        self.session = session
    ...
    def get_pids_from_name(self, name):
        """
        Get the PID of the specified process
        :param name: The process name
        :return: pid number list
        """
        result = self.session.run('ps -eo pid,cmd |grep %s |grep -v grep| awk \'{print $1}\'' % name)
    ...
    def get_startup_cmd_from_name(self, name):
        """
        Get the startup cmd of the specified process
        :param name: The process name
        :return: startup cmdline
        """
        result = self.session.run('ps -eo cmd |grep %s |grep -v grep' % name)
        if result.exit_status != 0:
            return None

        return result.stdout.split('\n')[0]

    def get_process_stat_from_pid(self, pid):
        """
        Get the status of the specified process
        :param name: The process pid
        :return: the process status
        """
        result = self.session.run('ps -eo pid,stat |grep %s |grep -v grep' % pid)
        if result.exit_status != 0:
            return None

        return result.stdout.split(' ')[1]

    def parent_process(self, pid):
        '''
        :param pid:  The process id
        :return: If the pid is primary process id, return True
        '''
        result = self.session.run('ps -eo pid,stat |grep %s |grep -v grep' % pid)
        if result.exit_status != 0:
            return False

        return result.stdout.split(' ')[1].__contains__('s')

    def exist_process(self, pid):
...
def get_remote_resource_usage(session=None, host=None, username=None,
                              password=None, use_key=False):
    session = session or RemoteRunner(host=host, username=username,
                                      password=password, use_key=use_key)
    resource_usage = dict()
    output = str(session.run('top -n3'))
    posi_start = output.rindex('%Cpu(s):')
    posi_end = output.index('\n', posi_start)
    cpu_line = output[posi_start:posi_end]
    patternFloatCpu = re.compile('\d+\.\d+')
    t_cpu_usage = re.findall(patternFloatCpu, cpu_line)
    cpu_usage = t_cpu_usage[0]
    resource_usage['cpu'] = float(cpu_usage)

    posi_start = output.rindex('KiB Mem :')
    posi_end = output.index('\n', posi_start)
    mem_line = output[posi_start:posi_end]
    patternFloatMem = re.compile(r'\d+')
    t_mem_usage = re.findall(patternFloatMem, mem_line)
    mem_usage = (float(t_mem_usage[2]) + float(t_mem_usage[3])) * \
                                               100 / float(t_mem_usage[0])
    resource_usage['mem'] = float(mem_usage) * 0.6
    resource_usage['mem_total'] = int(t_mem_usage[0])
...
def run_cmd_between_remotes(mid_host_ip, mid_host_user, mid_host_password,
    ...
    LOG.info('Connect to %s successfully !' % mid_host_ip)
    if cmd_mid:
        stdin, stdout, stderr = mid_cli.exec_command(cmd_mid)
        LOG.debug('stdout %s' % (''.join(stdout.readlines())))
        error_msg = ''.join(stderr.readlines())
        if error_msg != '':
            raise SSHException(error_msg)
        else:
            LOG.info('run cmd %s on %s successfully!' % (cmd, mid_host_ip))
        stderr.close()
        stdout.close()
    ...
    LOG.info('Connect to %s successfully !' % end_host_ip)
    ...
    stdout_msg = ''.join(stdout.readlines())
    LOG.info('stdout %s' % stdout_msg)
    error_msg = ''.join(stderr.readlines())
    if error_msg != '':
        raise SSHException('ERROR STATUS %s ; ERROR MSG %s' % (exit_status,
                                                               error_msg))
    else:
        LOG.info('run cmd %s on %s successfully!' % (cmd, end_host_ip))
...
def get_zabbix_server_ip(controller_ip, controller_user, controller_password):
    session = RemoteRunner(client='ssh', host=controller_ip,
                           username=controller_user, port="22",
                           password=controller_password)
    cmd = "grep \'api.inte.lenovo.com\' /etc/hosts |awk -F \' \' \'{print $1}\'"
    result = session.run(cmd)
    if result.exit_status != 0:
        raise SSHException(result.stderr)
    else:
        LOG.debug('Run cmd %s on %s successfully!' % (cmd, controller_ip))
    zabbix_server_ip = (result.stdout).strip('\n')
    LOG.info('Zabbix server ip is %s' % zabbix_server_ip)
    session.session.close()

    return zabbix_server_ip


def ping_host(controller_ip, controller_user, controller_password,
              host_ip, reachable=True, ipv6=False):
    session = RemoteRunner(client='ssh', host=controller_ip,
                           username=controller_user, port="22",
                           password=controller_password)
    if ipv6 == "True":
        cmd = "ping -6 %s -c 1 -W 30" % host_ip
    else:
        cmd = "ping %s -c 1 -W 30" % host_ip
    LOG.info('Run cmd %s on %s!' % (cmd, controller_ip))
    result = session.run(cmd, ignore_status=True)
    session.session.close()
    if result.exit_status != 0:
        LOG.info(result.stderr)
        LOG.info("Destination host unreachable!")
        return not reachable

    LOG.debug('Run cmd %s on %s successfully!' % (cmd, controller_ip))
    LOG.debug(result.stdout)
    if result.stdout.find('1 received'):
        LOG.info("Destination host reachable!")
        return reachable
    else:
        LOG.info("Destination host unreachable!")
        return not reachable
