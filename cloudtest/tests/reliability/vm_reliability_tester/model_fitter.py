...
def fit_models():
    ...
    print f_rates
    data = np.array(f_rates, dtype='float64')
    # need to exclude master vm
    vm_count = len(read_hosts_file("vm_list.csv")) - 1
    total = vm_count * 10 * 30
    print "vm count is: %d" % vm_count
    print "total execute count is: %d" % total
    data = data / total
    n = len(data)
    data.sort()
