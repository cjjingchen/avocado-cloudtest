...
def collect_data_files(vm_list):
    i = 1
    for vm in vm_list:
        get('/root/failures.csv.' + vm, 'failures_' + str(i) + '.csv', use_sudo=True)
        get('/root/response_time.csv.' + vm, 'response_time_' + str(i) + '.csv', use_sudo=True)
        i += 1
...
def data_collection():
    ...
    execute(run_python_program, program='/root/measurements_downloader.py', sudo=True)
