...
def install_execute_env(install_or_not):
    if install_or_not:
        execute(update)
        execute(upgrade)
...
def setup():
    ...
    ssh_private_key_filename = str(vm_credentials['ssh.key_filename'])
    ...
    execute(upload_file, '/root/.ssh/id_rsa', ssh_private_key_filename, sudo=True)
    execute(upload_file, '/root/.ssh/id_rsa.pub', ssh_public_key_filename, sudo=True)
...
if __name__ == "__main__":
    ...
    execute(upload_file, '/root/failures.csv', 'failures.csv', sudo=True)
    execute(upload_file, '/root/response_time.csv', 'response_time.csv', sudo=True)
    execute(run_python_program, program='/root/node_config.py', sudo=True)
    execute(upload_file, '/root/test_runner.py', 'test_runner.py', sudo=True)
    execute(upload_file, '/root/measurements_downloader.py', 'measurements_downloader.py', sudo=True)
    execute(upload_file, '/root/config_clearer.py', 'config_clearer.py', sudo=True)
