...
class VM_Control(object):
    ...
    def __init__(self):
        ...
        created_vm_info = dict(config['VM_INFO'])
        self.image_name = str(created_vm_info['image_name'])
        self.flavor_name = str(created_vm_info['flavor_name'])
        self.network_name = str(created_vm_info['network_name'])
    ...
    def create_vms(self, vms):
        return [self.create_vm(
            ...
            network_label=self.network_name,
            sec_group=self.sec_group,
            injected_pk=self.pk
        ) for vm in vms]
